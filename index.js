...
function batchGet(params, documentClient, notFoundMsg) {
  ...
  return batchGetImpl(params, documentClient).then(result => {
    tableNames.forEach(tableName => {
...
function get(params, documentClient, notFoundMsg) {
  return documentClient.get(params).then(data => {
    if (!data.Item) {
      throw new Error(notFoundMsg);
    }
...
function tryGet(params, documentClient) {
  return documentClient.get(params).then(data => data.Item || null);
}

module.exports = exports = function(options) {
  options = options || {};
  const documentClient = documentClientFactory.create(options);
