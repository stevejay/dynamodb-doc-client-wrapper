...
describe("BatchWriteParamsTakeManager", function() {
  const tests = [
    ...
    },
    {
      ...
      expected: [
        ...
        {
          RequestItems: {
            "Table-1": [{ PutRequest: { Item: { id: "1c" } } }],
            "Table-2": [{ PutRequest: { Item: { id: "2a" } } }]
          }
        },
        {
          RequestItems: {
  ...
  tests.forEach(test => {
    it(
      ...
      function() {
        const subject = new BatchWriteParamsTakeManager(
          test.args.params,
          test.args.maxTake
        );
        const result = [];
