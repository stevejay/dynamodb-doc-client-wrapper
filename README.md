NOTE: I created this package to make using the BatchGet and BatchWrite DynamoDB APIs easier to use. Nowadays I recommend that you use the `@aws/dynamodb-batch-iterator` package that the AWS team created. It's part of the AWS Labs [`dynamodb-data-mapper-js`](https://github.com/awslabs/dynamodb-data-mapper-js) project, which appears to be abandoned but has actually now been [brought under the auspices](https://github.com/aws/aws-sdk-js/issues/2637) of the official AWS SDK team. Hopefully that means using the `@aws/dynamodb-batch-iterator` package should be a solid long-term choice.
